...
int main(int argc, char *argv[])
{
    ...
    //perform handshake
    charsWritten = send(socketFD, "e", strlen("e"), 0); //send "e" to server
    if (charsWritten < 0)
    {
        error("CLIENT: ERROR writing to socket\n");
    }
    if (charsWritten < strlen("e"))
    {
        fprintf(stderr, "CLIENT: WARNING: Not all data written to socket!\n");
    }

    memset(handshakeText, '\0', sizeof(handshakeText)); 
    //expect to get "e*" back from server (opt_enc_d)
    ...
    if(strcmp("e*", handshakeText) != 0)
    {
        fprintf(stderr, "OTP_ENC: handshake failed at port %d, exiting now\n", portNumber);
        exit(2);
    }
